...
// Add ellipsoid visualization group
const ellipsoidGroup = xyGroup.append('g');
const ellipse = ellipsoidGroup.append('ellipse')
    .attr('stroke', 'green')
    .attr('fill', 'none')
    .attr('stroke-width', 1.5);

// Generate grid points for level set
const resolution = 100;
const theta1Range = d3.range(-Math.PI, Math.PI, 2*Math.PI/resolution);
const theta2Range = d3.range(-Math.PI, Math.PI, 2*Math.PI/resolution);

// Calculate function values
const levelSetPoints = [];
for (let theta1 of theta1Range) {
    for (let theta2 of theta2Range) {
        const value = Math.sin(theta2) - Math.sin(2*theta1 + 2*theta2);
        // Check if value is close to zero (within tolerance)
        if (Math.abs(value) < 0.001) {
            levelSetPoints.push({x: theta1, y: theta2});
        }
    }
}

// Draw level set points
thetaGroup.selectAll('.levelset')
    .data(levelSetPoints)
    .enter()
    .append('circle')
    .attr('class', 'levelset')
    .attr('cx', d => thetaScaleX(d.x))
    .attr('cy', d => thetaScaleY(d.y))
    .attr('r', 2)
    .attr('fill', 'orange');
...
svg.on('mousemove', (event) => {
    ...
    if (x * x + y * y <= 4) { // Inside the circle
        ...
        const J = calculateJacobian(theta1, theta2);
        const ellipsoidParams = calculateEllipsoid(J);
        ...
        ellipse
            .attr('cx', xyScaleX(x))
            .attr('cy', xyScaleY(y))
            .attr('rx', ellipsoidParams.major * scaleFactor * plotWidth)
            .attr('ry', ellipsoidParams.minor * scaleFactor * plotHeight)
            .attr('transform', `rotate(${-ellipsoidParams.angle * 180/Math.PI} ${xyScaleX(x)} ${xyScaleY(y)})`)
            .attr('visibility', 'visible');            
        ...
        ellipse.attr('visibility', 'hidden');
    }
});

// Function to calculate Jacobian matrix
function calculateJacobian(theta1, theta2) {
    const L1 = 1, L2 = 1;
    return [
...
// Function to calculate manipulability ellipsoid
function calculateEllipsoid(J) {
    const JJt = [
    ...
    // Calculate principal axis direction
    const theta = Math.atan2(lambda1 - JJt[0][0], JJt[0][1]);
